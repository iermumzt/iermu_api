...
parse_str(_authcode($code, 'DECODE', API_KEY), $get);
...
if($apptype == 'api') {
    ...
    $tablepre = API_DBTABLEPRE;
    $dbcharset = API_DBCHARSET;
    $db->connect(API_DBHOST, API_DBUSER, API_DBPW, API_DBNAME, $dbcharset, API_DBCONNECT, $tablepre);
} 
...
if($get['method'] == 'export') {
    ...
    if(trim($sqldump)) {
        $sqldump = "$idstring".
            ...
            "# $dbcharset\n".
            "# $apptype Home: http://www.icanmusic.com\n".
...
function encode_arr($get) {
    ...
    return _authcode($tmp, 'ENCODE', API_KEY);
}
