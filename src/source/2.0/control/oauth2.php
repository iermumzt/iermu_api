...
class oauth2control extends base {
    ...
    function ontoken() {
        $this->init_input();
        $this->log('oauth2222', 'start token, input='.json_encode($this->input));
        $_ENV['oauth2']->grantAccessToken();
    }
    
    function onauthorize() {
        $this->init_input();
        $this->log('oauth2222', 'start authorize, input='.json_encode($this->input));
        $filters = array(
            ...
            "bind_connect_type" => array("flags" => FILTER_REQUIRE_SCALAR),
            "bind_connect_uid" => array("flags" => FILTER_REQUIRE_SCALAR),
            "hardware" => array("flags" => FILTER_REQUIRE_SCALAR),
            "hardware_id" => array("flags" => FILTER_REQUIRE_SCALAR),
        );
        ...
        if(isset($input['redirect_uri'])) {
            $input['redirect_uri'] = rawurldecode($input['redirect_uri']);
        }
        
        // qrcode处理
        if ($input['response_type'] == OAUTH2_AUTH_RESPONSE_TYPE_QRCODE) {
            if (!$input["client_id"]) 
                $this->_error(API_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_CLIENT);
            ...
            if (!$input["response_type"])
                $this->_error(API_HTTP_BAD_REQUEST, OAUTH2_ERROR_UNSUPPORTED_RESPONSE_TYPE);

            // Check requested auth response type against the list of supported types
            if (array_search($input["response_type"], $_ENV['oauth2']->getSupportedAuthResponseTypes()) === FALSE)
                $this->_error(API_HTTP_BAD_REQUEST, OAUTH2_ERROR_UNSUPPORTED_RESPONSE_TYPE);
            
            $qrcode = $_ENV['oauth2']->createQRCode($input["client_id"], $input["scope"], $input["hardware"], $input["hardware_id"]);
            if(!$qrcode)
                $this->_error(API_HTTP_INTERNAL_SERVER_ERROR, OAUTH2_ERROR_QRCODE_CREATE_FAILED);
        ...
        */
        ...
        // bind校验
        if($input["bind_connect_type"] && $input["bind_connect_uid"]) {
            $connect = $this->db->fetch_first("SELECT * FROM ".API_DBTABLEPRE."member_connect WHERE connect_type='".$input["bind_connect_type"]."' AND connect_uid='".$input["bind_connect_uid"]."'");
            if($connect) {
                $uid = $connect['uid'];
                $this->set_login($uid);
        ...
        if(!$this->user || !$this->user['uid']) {
            ...
            if($connect_type > 0 && $this->connect_auth($input)) {
                ...
                */
                ...
                // 重定向到passport
                $url = PASSPORT_API.'/oauth2/authorize';
...
/*      } elseif($username != $_ENV['user']->replace_badwords($username)) {
    ...
    function show_error($error, $description="") {
        $this->log('oauth2222', 'show error, error='.$error);
    	//error code处理
    ...
    function onconnect_success() {
        ...
        $this->log('oauth2 connect success', 'code='.$code.', state='.$state);
        ...
        $this->log('oauth2 connect success', 'input='.json_encode($input));
        ...
        */
        ...
        $this->log('oauth2 connect success', 'connect_type='.$connect_type);
        
        $connect->set_state($state);
        $connect_token = $connect->get_connect_token();
        $this->log('oauth connect token', json_encode($connect_token));
        if(!$connect_token || !$connect_token['access_token'] || !$connect_token['refresh_token'] || !$connect_token['uid']) 
             $this->show_error(OAUTH2_ERROR_INVALID_REQUEST);
        
        $this->log('oauth2 connect success', 'connect_token='.json_encode($connect_token));
        ...
        $bind_connect_type = $input["bind_connect_type"];
        $bind_connect_uid = $input["bind_connect_uid"];
        
        if(!$uid) {
            if($bind_uid) {
                $this->showmessage('error', 'connect_bind_failed');
            } else if($bind_connect_type && $bind_connect_uid) {
                $this->showmessage('error', 'connect_bind_failed');
            } else {
                $this->show_error(OAUTH2_ERROR_INVALID_REQUEST);
            }
        ...
        if($bind_connect_type && $bind_connect_uid) {
            $c = $_ENV['user']->get_connect_by_uid($bind_connect_type, $uid);
            if($c) {
                if($bind_connect_uid != $c['connect_uid'] || $uid != $c['uid']) {
                    $this->showmessage('error', 'connect_alreay_bind_other');
                }
            } else {
                $this->db->query("INSERT INTO ".API_DBTABLEPRE."member_connect SET uid='$uid', connect_type='$bind_connect_type', connect_uid='$bind_connect_uid', dateline='".$this->base->time."', lastupdate='".$this->base->time."', status='1'");
            }
        }
        
        $this->log('oauth2 connect success', 'uid='.$uid);
