...
class adminbase extends base {
    ...
    function check_priv() {
        ...
        if(empty($username)) {
            header('Location: '.BASE_API.'/admin.php?m=user&a=login&iframe='.getgpc('iframe', 'G').($this->cookie_status ? '' : '&sid='.$this->view->sid));
            exit;
        } else {
            $this->user['isfounder'] = $username == 'APIAdministrator' ? 1 : 0;
            if(!$this->user['isfounder']) {
                $admin = $this->db->fetch_first("SELECT a.*, m.* FROM ".API_DBTABLEPRE."admins a LEFT JOIN ".API_DBTABLEPRE."members m USING(uid) WHERE a.username='$username'");
                if(empty($admin)) {
                    header('Location: '.BASE_API.'/admin.php?m=user&a=login&iframe='.getgpc('iframe', 'G').($this->cookie_status ? '' : '&sid='.$this->view->sid));
                    exit;
                } else {
                ...
                $this->user['username'] = 'APIAdministrator';
                $this->user['admin'] = 1;
    ...
    function log($action, $extra = '') {
        $log = htmlspecialchalog($this->user['username']."\t".$this->onlineip."\t".$this->time."\t$action\t$extra");
        $logfile = API_ROOT.'./data/logs/'.gmdate('Ym', $this->time).'.php';
        if(@filesize($logfile) > 2048000) {
            ...
            @rename($logfile, API_ROOT.'./data/logs/'.gmdate('Ym', $this->time).'_'.$hash.'.php');
        }
