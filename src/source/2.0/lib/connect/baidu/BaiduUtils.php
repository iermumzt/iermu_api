...
class BaiduUtils
{
    ...
    public static function request($url, $params = array(), $httpMethod = 'GET', $multi = false)
    {
        ...
        // var_dump($curl_opts);die;
        curl_setopt_array($ch, $curl_opts);
        $response = curl_exec($ch);

        $parts = explode("\r\n\r\nHTTP/", $response);
        $parts = (count($parts) > 1 ? 'HTTP/' : '').array_pop($parts);
        list($strHeader, $strResult) = explode("\r\n\r\n", $parts, 2);
        ...
        foreach ($arr as $key => $value) {
            ...
            $arrHeader[$temp[0]] = $temp[1];
        }
        
        if (empty($strResult)) {
            // log记录 ----------- error_log
            log::error_log('curl error', $url.$delimiter.http_build_query($params, '', '&'), $strResult, curl_errno($ch), curl_error($ch));
        ...
        self::setError(curl_getinfo($ch, CURLINFO_HTTP_CODE), $arrHeader['x-bs-request-id']);
        curl_close($ch);
