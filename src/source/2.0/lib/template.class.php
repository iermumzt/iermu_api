...
class template {
    ...
    function template() {
        ob_start();
        $this->defaulttpldir = API_ROOT.'./view/default';
        $this->tpldir = API_ROOT.'./view/default';
        $this->objdir = API_DATADIR.'./view/'.API_LANGUAGE;
        if(!file_exists($this->objdir)) {
            @mkdir($this->objdir, 0777, true);
        }
		$this->langfile = API_ROOT.'./view/locale/'.API_LANGUAGE.'/templates.lang.php';
    ...
    function complie() {
        ...
        $template = "<? if(!defined('API_ROOT')) exit('Access Denied');?>\r\n$template";
