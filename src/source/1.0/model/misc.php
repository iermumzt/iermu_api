...
define('API_ARRAY_SEP_1', 'API_ARRAY_SEP_1');
define('API_ARRAY_SEP_2', 'API_ARRAY_SEP_2');
...
class miscmodel {
    ...
    function check_client_id($client_id) {
        return preg_match(OAUTH2_CLIENT_ID_REGEXP, $client_id);
    }

    function check_client_secret($client_secret) {
        //return preg_match(OAUTH2_CLIENT_SECRET_REGEXP, $client_secret);
        return true;
    }
    ...
    function string2array($s) {
        $arr = explode(API_ARRAY_SEP_2, $s);
        $arr2 = array();
        foreach($arr as $k => $v) {
            list($key, $val) = explode(API_ARRAY_SEP_1, $v);
            $arr2[$key] = $val;
        }
        return $arr2;
    }
