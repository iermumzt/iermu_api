...
class mailmodel {
    ...
    function get_total_num() {
        $data = $this->db->result_first("SELECT COUNT(*) FROM ".API_DBTABLEPRE."mailqueue");
        return $data;
    }

    function get_list($page, $ppp, $totalnum) {
        $start = $this->base->page_get_start($page, $ppp, $totalnum);
        $data = $this->db->fetch_all("SELECT m.*, u.username, u.email FROM ".API_DBTABLEPRE."mailqueue m LEFT JOIN ".API_DBTABLEPRE."members u ON m.touid=u.uid ORDER BY dateline DESC LIMIT $start, $ppp");
    ...
    function delete_mail($ids) {
        ...
        $this->db->query("DELETE FROM ".API_DBTABLEPRE."mailqueue WHERE mailid IN ($ids)");
        return $this->db->affected_rows();
    }

    function add($mail) {
        if($mail['level']) {
            $sql = "INSERT INTO ".API_DBTABLEPRE."mailqueue (touid, tomail, subject, message, frommail, charset, htmlon, level, dateline, failures, appid) VALUES ";
            $values_arr = array();
            ...
            $insert_id && $this->db->query("REPLACE INTO ".API_DBTABLEPRE."vars SET name='mailexists', value='1'");
            return $insert_id;
        } else {
            $mail['email_to'] = array();
            $uids = 0;
            ...
            $users = $this->db->fetch_all("SELECT uid, username, email FROM ".API_DBTABLEPRE."members WHERE uid IN ($uids)");
            foreach($users as $v) {
    ...
    function _send() {
        ...
        $mail = $this->_get_mail();
        if(empty($mail)) {
            $this->db->query("REPLACE INTO ".API_DBTABLEPRE."vars SET name='mailexists', value='0'");
            return NULL;
        } else {
    ...
    function send_one_mail($mail) {
        ...
        return include API_SOURCE_ROOT.'lib/sendmail.inc.php';
    }

    function _get_mail() {
        $data = $this->db->fetch_first("SELECT m.*, u.username, u.email FROM ".API_DBTABLEPRE."mailqueue m LEFT JOIN ".API_DBTABLEPRE."members u ON m.touid=u.uid WHERE failures<'".API_MAIL_REPEAT."' ORDER BY level DESC, mailid ASC LIMIT 1");
        return $data;
    }

    function _get_mail_by_id($mailid) {
        $data = $this->db->fetch_first("SELECT m.*, u.username, u.email FROM ".API_DBTABLEPRE."mailqueue m LEFT JOIN ".API_DBTABLEPRE."members u ON m.touid=u.uid WHERE mailid='$mailid'");
        return $data;
    }

    function _delete_one_mail($mailid) {
        $mailid = intval($mailid);
        return $this->db->query("DELETE FROM ".API_DBTABLEPRE."mailqueue WHERE mailid='$mailid'");
    }

    function _update_failures($mailid) {
        $mailid = intval($mailid);
        return $this->db->query("UPDATE ".API_DBTABLEPRE."mailqueue SET failures=failures+1 WHERE mailid='$mailid'");
    }
}
