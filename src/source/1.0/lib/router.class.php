 ...
 */
    function connect($route, $defaults = array(), $options = array()) {
        ...
        if ($routeClass !== 'IRoute' && !is_subclass_of($Route, 'IRoute')) {
            trigger_error(__('Route classes must extend IRoute', true), E_USER_WARNING);
            return false;
        }
        $self->routes[] =& $Route;
...
/**
 * Specifies what named parameters should be parsing. The most common setups are:
 *
 * Do not parse any named parameters:
 *
 * {{{ router::connectNamed(false); }}}
 *
 * Parse only default parameters used for pagination:
 *
 ...
 *   is not in the root directory, this can be used to generate urls that are 'api relative'.
 *   api relative urls are required when using requestAction.
 * - `?` - Takes an array of query string parameters
 * - `#` - Allows you to set url hash fragments.
 * - `full_base` - If true the `FULL_BASE_URL` constant will be prepended to generated urls.
 *
 * @param mixed $url Api-relative URL, like "/products/edit/92" or "/presidents/elect/4"
 ...
 * are used for internals and should not normally be part of an output url.
 *
 ...
 *
 * @return IRoute Matching route object.
 * @access public
 * @static
 */
 ...
 * @access public
 */
    function IRoute($template, $defaults = array(), $options = array()) {
