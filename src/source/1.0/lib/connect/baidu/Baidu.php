...
class Baidu
{
     ...
     */
    public function getLoginUrl($scope = '', $display = 'page', $force_login = 1)
    {
        $oauth2 = $this->getBaiduOAuth2Service();
        return $oauth2->getAuthorizeUrl('code', $scope, $this->state, $display, $force_login);
    }
    ...
    {
        $session = $this->getSession();
        if (is_array($session) && isset($session['uid']) && isset($session['uname']) && isset($session['portrait'])) {
